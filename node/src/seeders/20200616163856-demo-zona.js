...
module.exports = {
  up: (queryInterface, Sequelize) => {
    let elementos=[];
    for (let index = 0; index < config.sizeDeliverys+config.sizeComercios; index++) {
      let p=[];
      ...
      p.push({lat: latitud+0.007, lng: longitud})
      p.push({lat: latitud+0.007, lng: longitud+0.007})
      p.push({lat: latitud, lng: longitud+0.007})
      elementos.push(
        {
